from . import MyNode, ChessFuncs, DepthFirst


## ################################
## SIMULATION
## ################################
def simulate(root: MyNode.Node, max_depth: int):
  # ChessFuncs.print5x5Board(root.board)
  queue_states = []
  for next_board in ChessFuncs.getNextBoardStates(root.board):
    queue_states.append({
      "depth": 1,
      "board": next_board,
      "parent": root,
  ...
  while len(queue_states) > 0:
    ...
    if (depth+1) > max_depth:
      return
    # ChessFuncs.print5x5Board(board, depth)
    node = MyNode.createNode(board, parent)
    ## TODO: if checkGameOver(board): continue
    for next_board in ChessFuncs.getNextBoardStates(board):
      if DepthFirst.checkNodeOccurance(root, next_board.fen().split(" ")[0]):
        continue
      queue_states.append({
        "depth":  depth+1,
        "board":  next_board,
...
#         child_dfi = DepthFirst.getNodeIndex(root, child.name, 0)
#         ## color child node
